...
lolevel_handler_irq: sub   lr, lr, #4              @ correct return address
                     sub   sp, sp, #60             @ update   SVC mode
                     stmia sp, { r0-r12, sp, lr }^ @ preserve USR
                     mrs   r0, spsr                @ move
                     stmdb sp!, { r0, lr }         @ store    USR PC and CPSR

                     mov   r0, sp                  @ set    high-level C function arg.
                     ldr   r1, [ lr, #-4 ]         @ load                     svc
                     bic   r1, r1, #0xFF000000     @ set    high-level C function arg. = svc
                     bl    hilevel_handler_irq     @ invoke high-level C function

                     ldmia sp!, { r0, lr }         @ load     USR mode PC and
                     msr   spsr, r0                @ move     USR
                     ldmia sp, { r0-r12, sp, lr }^ @ restore  USR mode
                     add   sp, sp, #60             @ update   SVC mode
                     movs  pc, lr                  @ return from interrupt

lolevel_handler_svc: sub   lr, lr, #0              @ correct return
                     sub   sp, sp, #60             @ update   SVC mode
                     stmia sp, { r0-r12, sp, lr }^ @ preserve USR
                     mrs   r0, spsr                @ move
                     stmdb sp!, { r0, lr }         @ store    USR PC and CPSR
